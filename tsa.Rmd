...
```{r load libraries and datasets, echo=FALSE}
library(tidyverse)
library(forecast)
library(tseries)
library(data.table)
...
##Plot
ggplot(datap)+ geom_line(aes(x=Year, y=value ))
...
Recall the autocorrelation function formula:
$\rho _k = \frac{\gamma_k}{\gamma_o}=\frac{Cov[y_t,y_{t-k}]}{Var(y_t)} $
